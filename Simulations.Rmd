...
ggplot() +
  geom_histogram(aes(oldGap) col = 2, bins = 50) +
  geom_histogram(aes(newGap), col = 4, bins = 50) 

c(mean(oldGap), mean(newGap))
```
...
calcTotalPoints <- function(season){
  totalPoints <- rep(0, 30)
  for (game in 1:length(seasonGames[[1]])){
    totalPoints[season[[2]][game]] = totalPoints[season[[2]][i]] +   season[[7]][game]
    totalPoints[season[[3]][game]] = totalPoints[season[[3]][i]] + season[[8]][game]
  }
  returnVec = data.frame(totalPoints, "Team Name" = c("New Jersey Devils", "NY Islanders", "NY Rangers", "Philadelphia Flyers", "Pittsburgh Penguins", "Boston Bruins", "Buffalo Sabres", "Montreal Canadiens", "Ottowa Senators", "Toronto Maple Leafs", "Winnipeg Jets", "Carolina Hurricanes", "Florida Panthers", "Tampa Bay Lightning", "Washington Capitals", "Chicago Blackhawks", "Detroit Red Wings", "Nashville Predators", "St Louis Blues", "Calgary Flames", "Colorado Avalanche", "Edmonton Oilers", "Vancouver Canucks", "Anaheim Ducks", "Dallas Stars", "LA Kings", "Phoenix Coyotes", "San Jose Sharks", "Columbus Blue Jackets", "Minnesota Wild"))
...
returnOldEastern <- function(points) {
  ...
  return(rbind(atlantic[1:2,],northeast[1:2,],southeast[1:2,],wildcards[1:2,],wildcards[3:9,]))
}

returnOldWestern <- function(points) {
  central = points[points$id %in% c(16:19, 29),]
  northwest = points[points$id %in% c(20:23, 30),]
  pacific = points[points$id %in% c(26:28),]
  ...
  return(rbind(central[1:2,],northwest[1:2,],pacific[1:2,],wildcards[1:2,],wildcards[3:9,]))
}
...
returnNewEastern <- function(points) {
  ...
  return(rbind(metro[1:3,],atlantic[1:3,],wildcards[1:2,],wildcards[3:10,]))
}
...
returnNewWestern <- function(points) {
  ...
  return(rbind(central[1:3,],pacific[1:3,],wildcards[1:2,],wildcards[3:8,]))
}
```{r}
