...
ggplot(df, aes(x = gap, fill = Rule_Type)) +
  ...
  geom_vline(aes(xintercept = mean(oldGap)), col = 4) +
...
returnOldEastern <- function(points) {
  ...
  top8 <- rbind(atlantic[1:2,],northeast[1:2,],southeast[1:2,],wildcards[1:2,])
  top8 = top8[order(-top8$totalPoints),]

  return(rbind(top8, wildcards[3:9,]))
}
...
returnOldWestern <- function(points) {
  ...
  top8 <- rbind(central[1:2,],northwest[1:2,],pacific[1:2,],wildcards[1:2,])
  top8 = top8[order(-top8$totalPoints),]
...
returnNewEastern <- function(points) {
  ...
  top8 <- rbind(metro[1:3,],atlantic[1:3,],wildcards[1:2,])
  top8 = top8[order(-top8$totalPoints),]

  return(rbind(top8, wildcards[3:10,]))
}
...
returnNewWestern <- function(points) {
  ...
  top8 <- rbind(central[1:3,],pacific[1:3,],wildcards[1:2,])
  top8 = top8[order(-top8$totalPoints),]

  return(rbind(top8, wildcards[3:8,]))
}
```
